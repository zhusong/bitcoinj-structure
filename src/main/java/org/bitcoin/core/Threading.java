...
public class Threading {
    ...
    public static class UserThread extends Thread implements Executor {
        ...
        public void run() {
            while (true) { //一直循环，阻塞处理
                Runnable task = Uninterruptibles.takeUninterruptibly(tasks);
                try {
                    task.run();
                } catch (Throwable throwable) {
                    //如果任务执行过程中出现异常，它会记录异常日志并将异常传递给Thread.UncaughtExceptionHandler处理。
    ...
    public static ListeningExecutorService THREAD_POOL = MoreExecutors.listeningDecorator(
            Executors.newCachedThreadPool(new ThreadFactory() { //线程工厂
                @Override
                public Thread newThread(Runnable r) { //创建线程，名字叫做Threading.THREAD_POOL worker，并且是Daemon，后台线程
                    Thread t = new Thread(r);
                    t.setName("Threading.THREAD_POOL worker");
                    t.setDaemon(true); //不会阻止程序的正常终止。当所有的非守护线程都结束时，守护线程会自动停止运行。
                    return t;
                }
            })
    );
